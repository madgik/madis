...
select jgroupunion(c1) from (select * from (xmlparse root:article fast:1 file 'dblp.xml.gz'));
...
select jgroupunion(c1) from (select * from (xmlparse root:www fast:1 file 'dblp.gz'));
...
select jgroupunion(c1) from (select * from (xmlparse root:inproceedings fast:1 file 'dblp.xml.gz'));
